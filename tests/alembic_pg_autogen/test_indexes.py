# These tests call the private functions of the comparator directly.
...
def _upgrade_body(content: str) -> str:
    ...
    Each operation from this comparator is reversible, so the same call text is also in ``downgrade()``. Thus each
    assertion about a migration must name the direction.
    """
...
def _t_table(*indexes: Index, metadata: MetaData | None = None) -> Table:
    return Table(
        ...
        # Use Text, not String. The table DDL below uses ``text`` columns. A mismatch makes Alembic emit an
        # ``alter_column``, which changes the column types that the deparse of the predicate uses.
        Column("a", Text()),
        Column("b", Text()),
        Column("status", Text()),
        Column("data", JSONB()),
...
class _StubAutogenContext:
    """A small replacement for ``AutogenContext`` with only the attributes that the comparator uses."""
...
def _stub_context(connection: object | None = None, **opts: object) -> Any:
    """Return a stub with the type ``Any``, so it can replace ``AutogenContext`` with no cast."""
...
class TestComparatorSkips:
    """Each case where the comparator emits no operation."""

    @pytest.fixture
    def catalog(self, monkeypatch: pytest.MonkeyPatch):
        """Replace the catalog and canonicalization calls, so the comparator runs with no database."""
    ...
    def test_uniqueness_difference_is_a_change(self, catalog: Any, monkeypatch: pytest.MonkeyPatch):
        ...
        catalog({"ix_t_a": "USING btree (a)"}, {})
        # Replace the canonicalizer again, so uniqueness is the only difference.
        monkeypatch.setattr(module, "canonicalize_indexes", fake_canonicalize)
        table = _t_table(Index("ix_t_a", "a"))
    ...
    def test_index_absent_from_the_reflected_table_is_skipped(self, catalog: Any):
        """Both operations need a reflected Index. If Alembic did not reflect the name, there is nothing to drop."""
        probed = catalog({"ix_t_a": "USING btree (a)"}, {"ix_t_a": "USING btree (lower(a))"})
...
class TestComparatorShortCircuits:
    """Cases that the comparator rejects before it reads the catalog."""
...
class TestIndexAutogenerateIntegration:
    """Each missed case, tested against a live PostgreSQL, and the equivalent cases that must give no operation."""
    ...
    def test_changed_definition_produces_drop_and_create(
        ...
        """Alembic alone emits no operation for each of these cases, on each run."""
    ...
    def test_equivalent_definition_produces_no_index_ops(
        ...
        """These definitions differ as text, but they are the same index."""
        self._project_with(alembic_project, index_ddl)
        metadata = MetaData()
        _t_table(metadata_index(), metadata=metadata)
    ...
    def test_one_drop_create_pair_when_alembic_also_detects_the_change(self, alembic_project: AlembicProject):
        """Alembic compares expressions. The two comparators must not both emit operations."""
    ...
    def test_canonicalization_leaves_no_probe_indexes_behind(self, alembic_project: AlembicProject):
        ...
        # ``alembic_version`` has its own primary key index. No index from the test may remain.
        assert names == {"t_pkey", "ix_t_a", "alembic_version_pkc"}


@pytest.mark.integration
class TestIndexMigrationRoundTrip:
    """Generate, apply, and generate again. The migration must run, and the second run must give no operation."""
    ...
    def test_concurrent_migration_applies(self, alembic_project: AlembicProject):
        """The autocommit block is necessary. PostgreSQL refuses CONCURRENTLY inside a transaction."""
        alembic_project.execute(
            "CREATE TABLE t (id serial PRIMARY KEY, a text, b text, status text, data jsonb, deleted_at timestamptz)"
        )
