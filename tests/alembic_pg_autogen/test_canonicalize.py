...
class TestCanonicalizeIndexesIntegration:
    """The desired side of index comparison, with a round trip through an empty copy of the table."""
    ...
    def test_every_index_unusable_skips_the_read_back(self, pg_conn: Connection):
        """If no test index is created, the function reads nothing back and returns an empty result, not an error."""
        from sqlalchemy import Index
    ...
    def test_uncompilable_index_does_not_abort_the_run(self, pg_conn: Connection, caplog: pytest.LogCaptureFixture):
        """``execute()`` raises ``CompileError`` before the server gets the statement. It is not a ``DBAPIError``.

        When the code caught only the server error, one such index stopped autogenerate. The other indexes on the
        table were then not compared.
        """
        ...
        # A JSONB bind has no literal renderer, so SQLAlchemy cannot compile this index.
    ...
    def test_clone_name_already_taken_reports_the_cause(self, pg_conn: Connection, caplog: pytest.LogCaptureFixture):
        """The copy must have the name of the target table, or the DDL does not find it. Thus the name must be free."""
        from sqlalchemy import Index
        ...
        # The existing temporary table does not change.
        columns = (
            pg_conn
            .execute(
                text(
    ...
    def test_failing_read_back_is_treated_as_unchanged(
        ...
        """The test indexes can succeed while the read back fails. That failure must not propagate."""
        from sqlalchemy import Index
    ...
    def test_probe_query_strips_a_postgresql_14_style_table_reference(self, pg_conn: Connection):
        """PostgreSQL 14 writes the real name ``pg_temp_3``. PostgreSQL 15 and later write ``pg_temp``.

        CI runs PostgreSQL 14, so the qualified prefix is the branch that matches there. Without that branch, no index
        could be normalized. ``pg_get_indexdef()`` always writes the schema of the table. On this server, a temporary
        table always gets ``pg_temp``. Thus the test uses a table in a named schema. Its output has the same form as
        the PostgreSQL 14 output for a temporary table.
        """
        from alembic_pg_autogen.canonicalize import _INDEX_PROBE_QUERY  # pyright: ignore[reportPrivateUsage]

        # Remove the filter for temporary tables and disable the ``pg_temp`` prefix. Then only the qualified prefix
        # can match.
    ...
    def test_unstrippable_probe_definition_is_treated_as_unchanged(
        ...
        """This branch hid the PostgreSQL 14 bug. A shape with no prefix match must log a warning."""
        from sqlalchemy import Index

        # The package exports the ``canonicalize`` *function*, which hides the submodule with the same name. Both
        # ``from alembic_pg_autogen import canonicalize`` and ``import alembic_pg_autogen.canonicalize as ...`` give
        # the function. Thus the test gets the module through ``importlib``.
        canonicalize_module = importlib.import_module("alembic_pg_autogen.canonicalize")
    ...
    def test_probe_query_yields_null_when_no_candidate_matches(self, pg_conn: Connection):
        """An unknown spelling must give NULL. The caller then reports the index as unchanged."""
        from alembic_pg_autogen.canonicalize import _INDEX_PROBE_QUERY  # pyright: ignore[reportPrivateUsage]
